...
if ( !isset($_FILES['files']) ) {
	OC_JSON::error(array("data" => array("message" => "No file was uploaded")));
	exit();
}
foreach ($_FILES['files']['error'] as $error) {
	if ( $error != 0 ) {
		$l = new OC_L10N('files');
		$errors = array(0 => $l -> t("There is no error, the file uploaded with success"), 1 => $l -> t("The uploaded file exceeds the upload_max_filesize directive in php.ini") . ini_get('upload_max_filesize'), 2 => $l -> t("The uploaded file exceeds the MAX_FILE_SIZE directive that was specified in the HTML form"), 3 => $l -> t("The uploaded file was only partially uploaded"), 4 => $l -> t("No file was uploaded"), 6 => $l -> t("Missing a temporary folder"));
...
if ( strpos($dir, '..') === false ) {
	...
	for ($i = 0; $i < $fileCount; $i++) {
		$target = stripslashes($dir) . trim($files['name'][$i]);
		if ( OC_Filesystem::fromUploadedFile($files['tmp_name'][$i], $target) ) {
			$result[] = array("status" => "success", 'mime' => OC_Filesystem::getMimeType($target), 'size' => OC_Filesystem::filesize($target), 'name' => trim($files['name'][$i]));
		}
	}
