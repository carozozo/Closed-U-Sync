...
class OC_USER_LDAP extends OC_User_Backend {
	...
	function __construct() {
		...
		if (!empty($this -> ldap_host) && !empty($this -> ldap_port) && ((!empty($this -> ldap_dn) && !empty($this -> ldap_password)) || (empty($this -> ldap_dn) && empty($this -> ldap_password))) && !empty($this -> ldap_base) && !empty($this -> ldap_login_filter) && !empty($this -> ldap_display_name)) {
	...
	private function setQuota($uid) {
		...
		// user default Quota 3G=3221225472, 5G=5368709120, 6G=6442450944, 8G=8589934592, 10G=10737418240, 30G=32212254720, 20G=21474836480
		if (is_null(OC_Preferences::getValue($uid, 'files', 'quota'))) {
			$defaultLdapQuota = OC_Config::getValue('defaultLdapQuota', '6442450944', 'CONFIG_CUSTOM');
			OC_UserQuota::setUserQuota($uid, $defaultLdapQuota);
		}
	}
