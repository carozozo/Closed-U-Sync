...
function bindControlEvents(){
	$(document).on('click','#editor_save',doFileSave)
	.on('click','#editor_close',hideFileEditor)
	.on('click','#gotolineval',goToLine)
	.on('click','#editorsearchval',doSearch)
	.on('click','#clearsearchbtn',resetSearch)
	.on('click','#nextsearchbtn',nextSearchResult);

	// $("#editor_save").live('click',doFileSave);
	// $('#editor_close').live('click',hideFileEditor);
	// $('#gotolineval').live('keyup', goToLine);
	// $('#editorsearchval').live('keyup', doSearch);
	// $('#clearsearchbtn').live('click', resetSearch);
	// $('#nextsearchbtn').live('click', nextSearchResult);
}
...
function doFileSave(){
	if(editorIsShown()){
		...
		// Show saving spinner
		$("#editor_save").off('click',doFileSave);
		$('#save_result').remove();
		...
		$.post(OC.filePath('files_texteditor','ajax','savefile.php'), { filecontents: filecontents, path: path, mtime: mtime },function(jsondata){
			if(jsondata.status!='success'){
				...
				$('#editor_save').text(t('files_texteditor','Save'));
				$("#editor_save").on('click',doFileSave);
			}
		},'json');
	}
};
			...
			function(result){
				if(result.status == 'success'){
					...
					$('table').fadeOut('slow', function() {
						...
						$('#editor').text(result.data.filecontents).show();
