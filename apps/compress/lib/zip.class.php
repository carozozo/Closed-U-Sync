...
class OC_Compress {
	...
	public static function compressTarget($target){
		...
		if($zip->open($dirTarget . '/' . $archiveName, ZipArchive::CREATE) === TRUE){
		    if(!is_dir($oc_target)){
	    		$zip->addFile($oc_target, iconv('UTF-8','big5', basename($oc_target)));
			}else{
	...
	private static function addFolderToZip($dir, $zipObj, $zipdir=''){
		...
		if (is_dir($dir)) {
	        if ($dh = opendir($dir)) {
	            if(!empty($zipdir)){
	            	$zipObj->addEmptyDir(iconv('UTF-8','big5', $zipdir));
				}
	            while (($file = readdir($dh)) !== false) {
	                if(!is_file($dir . "/" . $file)){
	                    ...
	                    $zipObj->addFile($dir . "/" . $file, iconv('UTF-8','big5', $zipdir . $file));
	                }
	            }
	        }
	    }  
