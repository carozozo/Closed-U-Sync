...
/**
 * 檢查 Client IP 是否在允許接受連線範圍內
 * @param $allow_segment Array 允許的網段
 *        $allow_ip      Array 允許的IP
 * @return boolean
 */
function checkAllowIP( $allow_segment, $allow_ip )
{
    $allow_use_api = false;
    
    // 1. 先檢查 Client 的 IP 是否在網段內
    foreach ($allow_segment as $segment)
    {
        if (CIDR::IPisWithinCIDR($_SERVER['REMOTE_ADDR'], $segment))
        {
            $allow_use_api = true;
            break;
        }
    ...
    if (!$allow_use_api)
    {
        $allow_ip[] = $_SERVER['SERVER_ADDR'];    // 允許Server自身
