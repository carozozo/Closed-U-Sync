...
class OC_Appconfig {
	 ...
	 * @author Caro Huang
	 * @param app id
	 * @return array of appconfig
	 */
	public static function getConfigItemsByAppId($appId, $withoutDefault = true) {
		$query = OC_DB::prepare('SELECT * FROM *PREFIX*appconfig WHERE appid = ?');
		$result = $query -> execute(array($appId));
		$items = $result -> fetchAll();
		if ($withoutDefault) {
			foreach ($items as $index => $item) {
				if ($item['configkey'] == 'installed_version' || $item['configkey'] == 'enabled') {
					unset($items[$index]);
				}
			}
		}
