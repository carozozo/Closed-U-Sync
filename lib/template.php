...
function relative_modified_date($timestamp) {
	...
	if ($timediff < 60) {
		...
		return $l -> t('%d minutes ago', $diffminutes);
	}
	// else if($timediff < 7200) { return '1 hour ago'; }
	//else if($timediff < 86400) { return $diffhours.' hours ago'; }
	else if ((date('G') - $diffhours) > 0) {
		return $l -> t('today');
	} else if ((date('G') - $diffhours) > -24) {
		return $l -> t('yesterday');
	} else if ($timediff < 2678400) {
		return $l -> t('%d days ago', $diffdays);
	} else if ($timediff < 5184000) {
		return $l -> t('last month');
	} else if ((date('n') - $diffmonths) > 0) {
		return $l -> t('months ago');
...
class OC_Template {
	...
	public function fetchPage() {
		...
		if ($this -> renderas) {
			...
			if ($this -> renderas == "user") {
				...
				if (array_search(OC_APP::getCurrentApp(), array(
					'settings',
					'admin',
					'help'
				)) !== false) {
				...
				# 新增左邊目錄的分類標題( name 的值請對應/core/l10n/)
				OC_App::addNavigationEntryClass(array(
					'class' => 'settings',
					'order' => 1,
					"name" => 'Settings'
				));
				...
				$page -> assign("navigationClass", OC_App::getNavigationClass());
				$page -> assign("navigation", OC_App::getNavigation());
				$page -> assign("settingsnavigation", OC_App::getSettingsNavigation());

				# 將 user id 及 email 傳送到前端頁面
				$userId = OC_User::getUser();
				$page -> assign("userId", $userId);
				$page -> assign("userEmail", OC_User::getUserEmail($userId));
				$page -> assign("userNickname", OC_User::getUserNickname($userId));
			} else if ($this -> renderas == "blank") {
				$page = new OC_Template("core", "layout.blank");
			} else {
			...
			foreach (OC_Util::$styles as $style) {
				if (strpos($style, 'http') === 0) {
					$page -> append("cssfiles", "$style.css");
				} else {
	 ...
	 *
	 * Includes another template. use <?php echo $this->inc('template');
