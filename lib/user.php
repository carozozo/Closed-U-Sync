...
class OC_User {
    ...
    public static function createUser($uid, $password) {
        ...
        $run = true;
        OC_Hook::emit("OC_User", "pre_createUser", array(
            "run" => &$run,
            "uid" => $uid,
            "password" => $password
        ));
        ...
        if ($run) {
            ...
            foreach (self::$_usedBackends as $backend) {
                ...
                # 設置預設時區
                $timezone = OC_Config::getValue('defaultTimezone', 'Asia/Taipei', 'CONFIG_CUSTOM');
                OC_Preferences::setValue($uid, 'calendar', 'timezonedetection', 'true');
                OC_Preferences::setValue($uid, 'calendar', 'timezone', $timezone);
                # 設置預設語系
                $lang = OC_Config::getValue('defaultLanguage', 'zh_TW', 'CONFIG_CUSTOM');
                OC_Preferences::setValue($uid, 'core', 'lang', $lang);

                OC_Hook::emit("OC_User", "post_createUser", array(
                    "uid" => $uid,
                    "password" => $password
                ));
                return true;
    ...
    public static function deleteUser($uid) {
        $run = true;
        OC_Hook::emit("OC_User", "pre_deleteUser", array(
            "run" => &$run,
            "uid" => $uid
        ));
        ...
        if ($run) {
            ...
            $now = OC_Helper::formatDateTimeLocalToUTC(null, 'YmdHis');
            $userFolderPath = OC::$CONFIG_DATADIRECTORY_ROOT . '/' . $uid;
            $userFolderPath = OC_Helper::pathForbiddenChar($userFolderPath);
            $newUserfolderPath = OC::$CONFIG_DATADIRECTORY_ROOT . '/#' . $uid . '#' . $now;
            rename($userFolderPath, $newUserfolderPath);
    ...
    public static function login($uid, $password) {
        return DIFF_User::login($uid, $password);
    }
    ...
    /**
     * 如果有傳userId過來(通常是從API),則直接取用；沒有的話則回傳目前的Web登入者
     * @author 20130321 add by Caro.Huang
     * @return string uid or false
     */
    public static function getUserByUserInput($userId = null) {
        if ($userId) {
    ...
    public static function setPassword($uid, $password) {
        $run = true;
        OC_Hook::emit("OC_User", "pre_setPassword", array(
            "run" => &$run,
            "uid" => $uid,
            "password" => $password
        ));
        ...
        if ($run) {
            ...
            OC_Hook::emit("OC_User", "post_setPassword", array(
                "uid" => $uid,
                "password" => $password
            ));
            return true;
     ...
     */
    public static function getUsers($isLDAP = false) {
        $users = array();
        if ($isLDAP)
            unset(self::$_usedBackends['database']);
        foreach (self::$_usedBackends as $backend) {
    ...
    /**
     * 設置user的email(目前不支援p1環境)
     * @param user id, email
     */
    static function setUserEmail($uid, $email) {
        if (!$uid) {
            $uid = self::getUser();
        }
        return OC_Preferences::setValue($uid, 'settings', 'email', $email);
    }

    /**
     * 設置user的nickname(目前不支援p1環境)
     * @param user id, nickname
     */
    static function setUserNickname($uid, $nickname) {
        if (!$uid) {
            $uid = self::getUser();
        }
        $nickname = trim($nickname);
        return OC_Preferences::setValue($uid, 'settings', 'nickname', $nickname);
    }

    static function getUserEmail($uid = null) {
        if (!$uid) {
            $uid = self::getUser();
        }
        return DIFF_User::getUserEmail($uid);
    }

    static function getUserNickname($uid = null) {
        if (!$uid) {
            $uid = self::getUser();
        }
        return DIFF_User::getUserNickname($uid);
    }
    ...
    static function validateUserPass($username, $password) {
        return DIFF_User::validateUserPass($username, $password);
    }

    static function isPaidUser($uid) {
        if (!$uid) {
            $uid = self::getUser();
        }
        return DIFF_User::isPaidUser($uid);
    }
